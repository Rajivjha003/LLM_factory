...
- Avoid `DROP TABLE`, `DELETE FROM`, or `TRUNCATE` as a first fix."""
...
Interpretation:
- This gives evidence without using `DROP TABLE`, `DELETE FROM`, or `TRUNCATE`.
...
def build_examples(count: int, seed: int) -> list[dict]:
    ...
    while len(examples) < count:
        ...
        if pattern == 0:
            ...
            user = "The same purchase order has two totals after joining header and lines. How do I debug the grain?"
